...
export default function TrackingPage() {
  ...
  const create = async () => {
    ...
    try {
      await createTracking(validation.instrument.symbol, "manual");
  ...
  const trigger = async (item: TrackingItem) => {
    ...
    try {
      ...
      setNotice(`${item.ticker} 博主观点摘要已进入生成队列。`);
  ...
  return <div className="product-page watchlist-monitor-page">
    ...
    <div className="watchlist-scope-note"><AppIcon name="evidence" /><div><strong>它如何影响系统</strong><span>启用的标的会提高相关 Twitter 情报在首页的优先级，并限制研究助手和报告的检索范围。</span></div></div>
    ...
    {loading ? <PageLoading label="正在整理关注标的" /> : error ? <PageError detail={error} onRetry={load} /> : items.length === 0 ? <PageEmpty title="尚未建立研究范围" detail="添加第一个标的后，系统会优先整理 Twitter 博主对它的观点、风险和预测。" action={<button className="button-primary" onClick={() => setShowForm(true)}>添加标的</button>} /> : filtered.length === 0 ? <PageEmpty title="当前筛选下没有标的" detail="这些标的目前没有需要处理的变化。" /> : <div className="watchlist-monitor-grid">{filtered.map((item) => {
      ...
      const reportStatus = String(item.config?.report_status || monitor.latest_report?.status || "idle");
      return <article className={`watchlist-monitor-card is-${item.status}`} key={item.id}>
        ...
        <div className="card-actions"><Link className="button-primary" href={`/tweets?q=${encodeURIComponent(item.ticker)}`}>查看相关推文</Link><button className="button-secondary" disabled={busyId === item.id || ["queued", "generating"].includes(reportStatus)} onClick={() => void trigger(item)}>{["queued", "generating"].includes(reportStatus) ? "观点摘要生成中" : reportStatus === "failed" ? "重试观点摘要" : "生成观点摘要"}</button>{monitor.latest_report && <Link className="button-secondary" href={`/reports/${monitor.latest_report.id}`}>上次摘要</Link>}<button className="button-secondary" disabled={busyId === item.id} onClick={() => void toggle(item)}>{item.status === "active" ? "暂停过滤" : "恢复过滤"}</button><button className="text-danger" onClick={() => setDeleteTarget(item)}>移除</button></div>
      </article>;
    })}</div>}
    <ConfirmDialog open={!!deleteTarget} title="移出标的监控" message={`确定移除「${deleteTarget?.ticker ?? ""}」吗？已有简报不会被删除。`} confirmText="移除" variant="danger" onConfirm={remove} onCancel={() => setDeleteTarget(null)} />
  </div>;
}
