...
export interface BloggerListItem { handle: string; name: string; bio: string | null; avatar_url: string | null; followers_count: number; market_focus: string[] | null; credibility_score: number; score_status?: string; score_label?: string; sample_confidence?: number; verified_count: number; pending_count: number; hit_rate: number | null; }

export default function BloggerCard({ blogger, rank }: { blogger: BloggerListItem; rank?: number }) {
  const isRated = blogger.verified_count > 0 && blogger.score_status !== "no_data";
  ...
  return <Link href={`/sources/${encodeURIComponent(blogger.handle)}`} className="source-card">
    ...
    <div className="source-evidence"><div><span>预测评分</span><strong>{confidence ?? "—"}</strong></div><div><span>已验证预测</span><strong>{blogger.verified_count}</strong></div><div><span>历史命中率</span><strong>{blogger.hit_rate == null ? "—" : `${Math.round(blogger.hit_rate * 100)}%`}</strong></div></div>
    <p className="source-score-label">{blogger.score_label || (isRated ? "样本积累中" : "暂无已验证预测")}</p>
