...
def analyze_single_tweet_endpoint(
    tweet_id: str,
    _admin: User = Depends(get_current_admin),
    db: Session = Depends(get_db),
):
    ...
    acquired, lock_token = try_acquire(lock_key)
    if not acquired:
